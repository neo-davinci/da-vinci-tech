    ...
    >
      ...
      <div className='grid gap-1.5'>
        {payload.map((item, index) => {
          ...
          const indicatorColor = color || (p.payload as Record<string, unknown>)?.fill || p.color;

          return (
            <div
              key={String(p.dataKey || index)}
              className={cn(
                '[&>svg]:text-muted-foreground flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5',
                indicator === 'dot' && 'items-center'
              )}
            >
              {formatter && p.value !== undefined && p.name ? (
                formatter(p.value, String(p.name), p, index, p.payload as Record<string, unknown>)
              ) : (
                <>
                  {itemConfig?.icon ? (
                    <itemConfig.icon />
                  ...
                  >
                    <div className='grid gap-1.5'>
                      ...
                      <span className='text-muted-foreground'>
                        {itemConfig?.label || String(p.name ?? "")}
                      </span>
                    </div>
                    {p.value != null && (
          ...
          >
            {itemConfig?.icon && !hideIcon ? (
              ...
              <div
                ...
                style={{
                  backgroundColor: String(p.color ?? '#888')
                }}
              />
            )}
            {itemConfig?.label}
